...
Based on the IUP Scintilla Notebook example
...
void new_file(Ihandle* ih)
{
  ...
  IupSetAttribute(dlg, "TITLE", "Untitled - ScriptBasic IDE/Debugger");
...
void open_file(Ihandle* ih, const char* filename)
{
  ...
  if (str)
  {
    ...
    IupSetfAttribute(dlg, "TITLE", "%s - ScriptBasic IDE/Debugger", str_filetitle(filename));
...
void saveas_file(Ihandle* multitext, const char* filename)
{
  ...
  if (write_file(filename, str, count))
  {
    ...
    IupSetfAttribute(IupGetDialog(multitext), "TITLE", "%s - ScriptBasic IDE/Debugger", str_filetitle(filename));
    IupSetStrAttribute(multitext, "FILENAME", filename);
    IupSetAttribute(multitext, "DIRTY", "NO");
...
{
  IupMessage("About", "   ScriptBasic IDE/Debugger");
  return IUP_DEFAULT;
}
...
int main(int argc, char **argv)
{
  ...
  IupOpen(&argc, &argv);
  IupImageLibOpen();
  IupSetGlobal("DEFAULTFONTSIZE","10");

  IupScintillaOpen();

  config = IupConfig();
  IupSetAttribute(config, "APP_NAME", "ScriptBasic IDE/Debugger");
